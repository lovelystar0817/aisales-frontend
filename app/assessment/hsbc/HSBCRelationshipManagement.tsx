...
export default function HSBCRelationshipManagement() {
  const { t } = useTranslation();
  const { hsbcAdvisoryTechniqueData: data } = useAssessmentContext();

  if (!data) {
    return (
      <div className="rounded-2xl bg-white">
        <h2 className="mb-1 p-6 pb-0 text-[16px] font-bold">
          {t('assessment.relationshipManagement')}
        </h2>
        <FeedbackSkeleton />
      </div>
    );
  ...
  return (
    <section id="advisory-technique" className="rounded-2xl bg-[#FFFFFF] p-4">

...
export default function HSBCRepresentation() {
  const { t } = useTranslation();
  const { hsbcRepresentationData: data } = useAssessmentContext();

  if (!data) {
    return (
      <div className="rounded-2xl bg-white">
        <h2 className="mb-1 p-6 pb-0 text-[16px] font-bold">
          {t('assessment.hsbcRepresentation')}
        </h2>
        <FeedbackSkeleton />
      </div>
    );
  ...
  return (
    <section id="hsbc-representation" className="rounded-2xl bg-[#FFFFFF] p-4">

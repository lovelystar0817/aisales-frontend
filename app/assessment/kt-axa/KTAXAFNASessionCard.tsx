...
const KTAXAFNASessionCard = () => {
  ...
  const totalScores = useMemo(
    () => [
      {
        score: ktAxaSoftSkillsData?.overallScore,
        label: t('assessment.softSkills', 'Soft Skills'),
      },
      {
        score: ktAxaKnowledgeSkillsData?.overallScore,
        label: t('assessment.knowledgeSkills', 'Knowledge Skills'),
      },
      {
        score: ktAxaProductKnowledgeData?.overallScore,
  ...
  const handleListenToFeedback = () => {
    if (isAudioPlaying) {
      ...
      // Soft Skills section
      if (ktAxaSoftSkillsData) {
        feedbackSections.push(
          t('assessment.audioFeedback.softSkillsIntro', {
            score: ktAxaSoftSkillsData.overallScore,
            max: ktAxaSoftSkillsData.maxScore || 100,
            defaultValue: `Your soft skills score is ${ktAxaSoftSkillsData.overallScore} out of ${ktAxaSoftSkillsData.maxScore || 100}.`,
          }),
        );

        if (ktAxaSoftSkillsData.description) {
          feedbackSections.push(ktAxaSoftSkillsData.description);
        }
      }

      // Knowledge Skills section
      if (ktAxaKnowledgeSkillsData) {
        feedbackSections.push(
          t('assessment.audioFeedback.knowledgeSkillsIntro', {
            score: ktAxaKnowledgeSkillsData.overallScore,
            max: ktAxaKnowledgeSkillsData.maxScore || 100,
            defaultValue: `Your knowledge skills score is ${ktAxaKnowledgeSkillsData.overallScore} out of ${ktAxaKnowledgeSkillsData.maxScore || 100}.`,
          }),
        );

        if (ktAxaKnowledgeSkillsData.description) {
          feedbackSections.push(ktAxaKnowledgeSkillsData.description);
        }
      }

      // Product Knowledge section
      if (ktAxaProductKnowledgeData) {
        feedbackSections.push(
          t('assessment.audioFeedback.productKnowledgeIntro', {
            score: ktAxaProductKnowledgeData.overallScore,
            max: ktAxaProductKnowledgeData.maxScore || 100,
            defaultValue: `Your product knowledge score is ${ktAxaProductKnowledgeData.overallScore} out of ${ktAxaProductKnowledgeData.maxScore || 100}.`,
          }),
        );

        if (ktAxaProductKnowledgeData.description) {
          feedbackSections.push(ktAxaProductKnowledgeData.description);
        }
      }
  ...
  const isDataIncomplete =
    ...
    (!totalScores[2] ||
      totalScores[2].score === null ||
      totalScores[2].score === undefined);
        ...
        <header className="col-span-2 flex w-full items-start space-x-6 md:col-span-1 lg:col-span-2">
          {!isDataIncomplete ? (
            ...
            >
              ...
              />
              {/* Middle circle - Knowledge Skills */}
              ...
              {/* Inner circle - Product Knowledge */}
              <div className="absolute inset-0 flex items-center justify-center">
                {totalScores[2]?.score != null && (
                  <Circle
                    size={52}
            ...
            {!isDataIncomplete ? (
              <div>
                ...
                <ul className="space-y-2">
                  ...
                  {totalScores[2]?.score != null && (
                    <ScoreIndicator
                      score={totalScores[2].score}
                      label={totalScores[2].label}
                    />
                  )}
                </ul>
              </div>
...
export default KTAXAFNASessionCard;

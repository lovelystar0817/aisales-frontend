...
export default function KTAXAProductKnowledge() {
  const { t } = useTranslation();
  const { ktAxaProductKnowledgeData: data, session } = useAssessmentContext();

  const assessmentType = session?.assessmentType || 'kt-axa-fna';
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div className="relative flex items-center justify-between bg-white py-2">
          <div className="flex-1 pr-6">
            ...
            <p className="text-[14px] text-[#58595A]">
              {t(`assessment.${localeKey}.sectionDescriptions.productKnowledge`)}
            </p>
          </div>
        ...
        {data.sections?.map(
          ({ title, score, maxScore = 100, why, suggestion }: any) => (
            <div
              key={title}

...
export default function KTAXASoftSkills() {
  const { t } = useTranslation();
  const { ktAxaSoftSkillsData: data, session } = useAssessmentContext();

  const assessmentType = session?.assessmentType || 'kt-axa-recruitment';

  // Map assessment type to locale key
  const getLocaleKey = () => {
    if (assessmentType === 'kt-axa-recruitment') return 'ktAxaRecruitment';
    if (assessmentType === 'kt-axa-fna') return 'ktAxaFna';
    if (assessmentType === 'kt-axa-wealthplus') return 'ktAxaWealthplus';
    return 'ktAxaRecruitment'; // fallback
  };
  ...
  return (
    <section id="kt-axa-soft-skills" className="rounded-2xl bg-[#FFFFFF] p-4">
        ...
        <div className="relative flex items-center justify-between bg-white py-2">
          <div className="flex-1 pr-6">
            ...
            <p className="text-[14px] text-[#58595A]">
              {t(`assessment.${localeKey}.sectionDescriptions.softSkills`)}
            </p>
          </div>

...
interface MSIGTravelEasyAssessmentSectionProps {
  type: 'knowledgeSkills' | 'softSkills' | 'productKnowledge';
  data: SalesTechniquesParams | null | undefined;
  sectionId: string;
}

export function MSIGTravelEasyAssessmentSection({
  type,
  data,
  sectionId,
}: MSIGTravelEasyAssessmentSectionProps) {
  const { t } = useTranslation();

  const translationKey = `assessment.${type}`;
  ...
  const maxScore = type === 'productKnowledge' ? 40 : 30;
  const displayScore = (data.overallScore / maxScore) * 100;
      ...
      <div className="space-y-4">
        <div className="relative flex items-center justify-between bg-white py-2">
          <div className="flex-1 pr-6">
            <h2 className="mb-1 text-[16px] font-bold">{t(translationKey)}</h2>
          ...
          <div className="flex flex-row items-center space-x-2">
            ...
            <div className="relative">
              <Circle
                size={66}
                value={displayScore}
                color={getScoreRating(displayScore).color}
                bgColor="#E5E7EB"
              />

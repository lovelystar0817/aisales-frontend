...
export default function PrudentialObjectionHandlingLAPR() {
  const { t } = useTranslation();
  const { prudentialObjectionHandlingData } = useAssessmentContext();

  const data = prudentialObjectionHandlingData?.objectionHandling;

  if (!data) {
    return (
      <div className="rounded-2xl bg-white">
        <h2 className="mb-1 p-6 pb-0 text-[16px] font-bold">
          {t('assessment.objectionHandling')}
        </h2>
        <FeedbackSkeleton />
      </div>
    );
  ...
  return (
    <section id="objection-handling" className="rounded-2xl bg-[#FFFFFF] p-4">
      <div className="space-y-4">
        <div className="relative flex items-center justify-between bg-white py-2">
          <div className="flex-1 pr-6">
            ...
            <p className="text-[14px] text-[#58595A]">
              {data.description ||
                t('assessment.prudentialObjectionHandling.laprDescription')}
            </p>
          </div>
        ...
        {data.sections?.map(
          ({ title, score, maxScore = 25, why, suggestion }: any) => (
            <div
              key={title}

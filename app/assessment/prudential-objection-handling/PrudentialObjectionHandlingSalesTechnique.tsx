...
export default function PrudentialObjectionHandlingSalesTechnique() {
  const { t } = useTranslation();
  const { prudentialObjectionHandlingData } = useAssessmentContext();

  const data = prudentialObjectionHandlingData?.salesTechnique;
  ...
  return (
    <section id="sales-technique" className="rounded-2xl bg-[#FFFFFF] p-4">
      <div className="space-y-4">
        <div className="relative flex items-center justify-between bg-white py-2">
          <div className="flex-1 pr-6">
            ...
            <p className="text-[14px] text-[#58595A]">
              {data.description ||
                t('assessment.prudentialObjectionHandling.salesTechniqueDescription')}
            </p>
          </div>
        ...
        {data.sections?.map(({ title, score, maxScore, why, suggestion }) => (

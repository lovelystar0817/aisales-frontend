...
const PrudentialPHFactFindingSessionCard = () => {
  ...
  const totalScores = useMemo(() => {
    const factFindingRaw = prudentialPHFactFindingTechniqueData?.overallScore;
    const factFindingMax = prudentialPHFactFindingTechniqueData?.maxScore || 100;
    const productKnowledgeRaw = prudentialPHProductKnowledgeData?.overallScore;
    const productKnowledgeMax =
      prudentialPHProductKnowledgeData?.maxScore || 100;

    return [
      {
        score:
          factFindingRaw != null
            ? Math.round((factFindingRaw / factFindingMax) * 100)
            : undefined,
        label: t('assessment.factFindingTechnique', 'Fact Finding Technique'),
      },
      {
        score:
          productKnowledgeRaw != null
            ? Math.round((productKnowledgeRaw / productKnowledgeMax) * 100)
            : undefined,
      ...
      <article className="grid w-full grid-cols-2 items-start gap-4 space-y-6 rounded-2xl bg-white p-4 lg:w-[300px]">
        <header className="col-span-2 flex w-full items-start space-x-6 md:col-span-1 lg:col-span-2">
          {!isDataIncomplete ? (
            ...
            >
              {/* Outer circle - Fact Finding Technique */}
              <Circle
...
export default PrudentialPHFactFindingSessionCard;

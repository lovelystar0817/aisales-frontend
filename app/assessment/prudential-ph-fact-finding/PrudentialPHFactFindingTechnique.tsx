...
export default function PrudentialPHFactFindingTechnique() {
  const { t } = useTranslation();
  const { prudentialPHFactFindingTechniqueData: data } =
    ...
    return (
      <div className="rounded-2xl bg-white">
        <h2 className="mb-1 p-6 pb-0 text-[16px] font-bold">
          {t('assessment.factFindingTechnique', 'Fact Finding Technique')}
        </h2>
        <FeedbackSkeleton />
      </div>
    );
  ...
  return (
    <section id="fact-finding-technique" className="rounded-2xl bg-[#FFFFFF] p-4">
      <div className="space-y-4">
        <div className="relative flex items-center justify-between bg-white py-2">
          ...
          <div className="flex flex-row items-center space-x-2">
            ...
            <div className="relative">
              <Circle
                size={66}
                value={Math.round((data.overallScore / (data.maxScore || 100)) * 100)}
                color={getScoreRating(Math.round((data.overallScore / (data.maxScore || 100)) * 100), true).color}
                bgColor="#E5E7EB"
              />
        ...
        {data.sections?.map(({ title, score, maxScore, why, suggestion }: any) => (
          <div key={title} className="rounded-[12px] border border-[#D9DDE0] p-4">
            <div className="flex items-center justify-between bg-white py-2">
              ...
              <div className="text-[14px] font-bold text-[#161618]">
                {score}
                <span className="font-normal text-[#58595A]">/ {maxScore}</span>
              </div>
            </div>
            ...
            <ul className="space-y-2 pl-2">
              <li className="flex items-start text-[14px] font-normal text-[#58595A]">
                ...
                <div>
                  <strong className="font-bold">{t('assessment.why')}:</strong>{' '}
                  {why}
                </div>
              </li>
              <li className="flex items-start text-[14px] font-normal text-[#58595A]">

...
interface ProductKnowledgeProps {
  useLinearScore?: boolean;
}

export default function ProductKnowledge({
  useLinearScore = false,
}: ProductKnowledgeProps = {}) {
  const { t } = useTranslation();
  const { productKnowledgeData: data } = useAssessmentContext();
  ...
  return (
    <section id="product-knowledge" className="rounded-2xl bg-[#FFFFFF] p-4">
          ...
          {useLinearScore ? (
            <div className="flex items-center gap-2">
              <span className="text-[14px] font-bold text-[#161618]">
                {Math.round((data.overallScore / (data.maxScore ?? 100)) * 100)}
              </span>
              ...
              <div className="h-2 w-16 overflow-hidden rounded-full bg-gray-200">
                <div
                  ...
                  style={{
                    width: `${Math.round((data.overallScore / (data.maxScore ?? 100)) * 100)}%`,
                  }}
                />
              </div>
            </div>

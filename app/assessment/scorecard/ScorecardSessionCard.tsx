...
const ScorecardSessionCard = () => {
  ...
  // Calculate scores for display similar to original
  const totalScores = useMemo(
    () =>
      customSections.map((section: CustomSection) => ({
        score: section.overallScore > 0 ? Math.round(section.overallScore) : 0,
        label: section.name,
      })),
  ...
  // Calculate overall score from custom sections
  ...
  // SCORM completion handling - use backend overallScore when scormCompletionReady
  const scormCompletionReady = session?.roleplay?.scormCompletionReady;
  ...
  const { completionStatus, dismissCompletion } = useScormCompletion({
    overallScore:
      isScormReady && scormOverallScore != null
        ? scormOverallScore
        : overallScore,
    sessionId,
  ...
  const handleListenToFeedback = () => {
    if (isAudioPlaying) {
      ...
      // Custom Scorecards section
      if (customSections.length > 0) {
        feedbackSections.push(
          `Custom Assessment feedback: You scored ${overallScore} out of 100.`,
        );

        // Include individual custom section feedback
        customSections.forEach((section: CustomSection) => {
          feedbackSections.push(
            `For ${section.name}, you scored ${section.overallScore} out of ${section.maxScore}. ${section.reason || ''} ${section.suggestion || ''}`,
          );
        });
      }
...
export default ScorecardSessionCard;
